...
class ReleasePackagingTests(unittest.TestCase):
    ...
    def test_windows_archive_has_stable_name_root_and_hash(self) -> None:
        ...
        self.assertEqual(
            archive.name, "SDAD-Inspector-0.0.1-alpha.2-windows-x64.zip"
        )
    ...
    def test_macos_archive_contains_one_portable_executable(self) -> None:
        ...
        archive = Path(str(evidence["archive"]))
        self.assertEqual(
            archive.name, "SDAD-Inspector-0.0.1-alpha.2-macos-arm64.tar.gz"
        )
    ...
    def test_checksum_manifest_requires_one_archive_per_platform(self) -> None:
        ...
        for name in (
            "SDAD-Inspector-0.0.1-alpha.2-linux-x64.tar.gz",
            "SDAD-Inspector-0.0.1-alpha.2-macos-arm64.tar.gz",
            "SDAD-Inspector-0.0.1-alpha.2-windows-x64.zip",
        ):
