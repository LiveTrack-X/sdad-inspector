...
release pass. The product package version is `0.0.1a3`, the immutable release
Git tag is `v0.0.1-alpha.3`, and the public release title is
`SDAD Inspector 0.0.1 alpha`. The authoritative owner decision is recorded once
in `docs/claim-registry.md`.

The earlier `v0.0.1-alpha.1` and `v0.0.1-alpha.2` tags are retained as failed
validation history: their workflows published no GitHub Release or assets and
the tags are never moved.
