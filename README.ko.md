...
1. [`v0.0.3` 릴리스](https://github.com/LiveTrack-X/sdad-inspector/releases/tag/v0.0.3)를 엽니다.
2. 내 컴퓨터에 맞는 압축 파일과 `SHA256SUMS`를 받습니다.
3. 아래 방법으로 해시를 확인합니다.
4. 압축을 풀고 실행 파일을 엽니다.
5. 안내가 표시되면 `sdad-state.yaml`이 있는 프로젝트 루트를 고릅니다.
...
Inspector 창이 먼저 열립니다. 이전에 열었던 유효한 프로젝트가 있으면 가장 최근
항목을 자동으로 불러옵니다. 처음 실행한 컴퓨터에서는 Inspector UI가 나타난 뒤
앱 안의 프로젝트 선택 창을 보여 줍니다. 운영체제 폴더 선택 창은 **찾아보기**를
눌렀을 때만 열립니다. 이후에는 상단 폴더 버튼으로 언제든 프로젝트를 바꿀 수 있습니다.
...
화면입니다. 우측 상단 언어 메뉴에서 English, 한국어, 日本語, 简体中文을 선택할 수
있습니다. 테마와 UI 확대 비율은 Inspector 전용 사용자 설정에 저장됩니다.*

README 맨 위의 한 줄 제품 배너는 저장소 소개용입니다. 프로그램 `개요` 화면에는
표시하지 않으며, 작업 공간은 활성 패킷과 근거부터 바로 시작합니다.

- **상단 바** — 현재 프로젝트와 번들 SDAD 엔진을 확인하고, 수동/AUTO 15초
  재검사, 폴더 열기, 경로 복사, 언어, 테마, 90–150% UI 확대/축소를 조작합니다.
  기본 크기는 110%이며 `Ctrl/Cmd` + `+`, `-`, `0` 단축키도 지원합니다.
...
- **증거 항목** — 출처 메타데이터를 먼저 보여 주고 바로 아래에서 범위가 제한된
  원문 본문을 읽을 수 있습니다. Doctor/스냅샷은 JSON, 상태는 YAML, 저장소 문서는
  안전한 Markdown 뷰어로 표시합니다.
...
`0.0.3` 포터블 앱에는 검증된 `official-sdad-3` 어댑터와 SDAD 3.2.2 엔진만
...
새 앱이 정상적으로 시작되면 인증된 UI가 성공 핸드오프를 한 번 확인하고, 현재
실행 파일과 정확히 짝이 맞는 `.previous` 백업과 성공 표식을 정리합니다. 완료
알림은 닫을 수 있고 잠시 뒤 자동으로 사라지므로 다음 실행에서 반복되지 않습니다.
...
- 현재 SPEC·패킷·TODO·발견사항·owner gate·선언된 검증·Git 관찰값을 한 화면에
  정규화합니다.
- 한국어/English/日本語/简体中文, 라이트/다크, 넓은 화면/좁은 화면에서 같은 React UI를 사용합니다.
...
최근 프로젝트 목록, 언어/테마/UI 확대 비율/패널 폭, 제품 업데이트 staging은 OS별 사용자 앱
...
`v0.0.3`의 EXE에는 SDAD Inspector 로고와 제품 버전 정보가 들어 있습니다. 다만
Windows 탐색기는 실행 파일의 전체 경로를 기준으로 아이콘을 캐시하기 때문에, 같은
폴더의 같은 `SDAD-Inspector.exe`를 수동 교체하면 실행 전에는 예전 PyInstaller
아이콘이 남아 있을 수 있습니다.

1. `v0.0.3`를 한 번 실행합니다. 정상적인 Windows 포터블 시작은 현재 EXE의 정확한
   경로와 아이콘 연결을 셸에 다시 알립니다.
2. 바탕 화면을 선택하고 <kbd>F5</kbd>를 눌러 표시를 새로 고칩니다.
3. 파일 **속성 → 자세히**에서 제품 이름 `SDAD Inspector`, 제품 버전 `0.0.3`을
   확인합니다.
4. 실행 전부터 확인해야 한다면 릴리스 압축을 새 폴더에 풀어, 캐시 이력이 없는
   경로의 아이콘을 비교합니다.

자동 업데이트도 검증된 교체 직후 같은 경로 갱신을 요청합니다. 이 새로 고침은
표시 전용이며, 실패해도 프로그램 시작·업데이트 성공·롤백을 막지 않습니다. 아이콘
표시는 실행 파일의 동작이나 SHA-256 검증 결과와 별개입니다.
...
첫 실행에서는 GUI가 열린 뒤 앱 안의 프로젝트 선택 창이 표시됩니다.
...
- [`README.md`](README.md) — 기본 영문 안내
- [`README.ko.md`](README.ko.md) — 이 한국어 안내
- [`README.ja.md`](README.ja.md) — 일본어 사용 안내
- [`README.zh-CN.md`](README.zh-CN.md) — 중국어 간체 사용 안내
...
- [`docs/releases/v0.0.3.md`](docs/releases/v0.0.3.md) — 현재 릴리스의 시작 흐름,
...
정확한 `v0.0.3` 태그는 같은 검증을 다시 수행한 뒤 세 플랫폼 압축과
...
SDAD Inspector는 [MIT License](LICENSE)로 제공됩니다. 프로젝트가 도움이 되었다면
[GitHub Sponsors](https://github.com/sponsors/LiveTrack-X)에서 유지보수를 후원할 수
있습니다. 후원 여부는 라이선스, 지원 범위, 릴리스 근거를 바꾸지 않습니다.
