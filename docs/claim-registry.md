...
| CLAIM-SI-007 | Exact same-commit Windows X64, macOS ARM64, and Linux X64 hosted artifacts built and passed full contracts plus direct and separate downloaded-archive smoke | allowed_with_qualifier | P2_qualified | Developer docs, release notes | EVID-SI-004, EVID-SI-014 | `exact unsigned v0.0.1-alpha.3 artifacts on the recorded GitHub-hosted runner images` | Product support headline, stable-release claim | Hosted-runner release evidence is not a general operating-system support commitment |
...
| CLAIM-SI-015 | The 0.0.1 alpha release is a portable single-file application that needs no separately installed Python runtime | allowed_with_qualifier | P2_qualified | README, release notes, developer docs | EVID-SI-014 | `exact unsigned v0.0.1-alpha.3 artifacts and tested hosted-runner environments only; operating-system webview/display components are still required` | Broad OS support headline, stable/signed/notarized/installer claim | Each public archive contains one executable and passed a separate clean-runner launch; this does not remove documented operating-system component requirements |
...
- `OWNER-SI-013-PORTABLE-002` — On 2026-07-15 the owner clarified that the
  ...
  Outcome: tagged run `29413712979` published the one authorized unsigned
  prerelease at `https://github.com/LiveTrack-X/sdad-inspector/releases/tag/v0.0.1-alpha.3`
  with three single-member platform archives and `SHA256SUMS`; fresh public
  downloads were independently hash- and member-verified.
