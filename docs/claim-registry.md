...
| CLAIM-SI-007 | Exact same-commit Windows X64, macOS ARM64, and Linux X64 hosted preview artifacts built and passed direct plus separate downloaded-archive smoke | allowed_with_qualifier | P2_qualified | Developer docs, candidate report | EVID-SI-004, EVID-SI-014 | `exact unsigned commit 0466e76 on current GitHub-hosted runner images; tag and published assets pending` | Product support headline, stable-release claim | Hosted-runner candidate evidence is not a general operating-system support commitment |
